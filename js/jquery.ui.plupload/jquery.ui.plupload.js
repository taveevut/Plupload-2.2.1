...
@example
	...
	$('#uploader').plupload('notify', 'info', "This might be obvious, but you need to click 'เพิ่มไฟล์' to add some files.");
			...
			'<div class="ui-widget-content plupload_container">' +
				'<div class="ui-state-default ui-widget-header plupload_header">' +
					'<div class="plupload_header_content">' +
						...
						'<div class="plupload_header_title">' + _("เลือกไฟล์") + '</div>' +
						'<div class="plupload_header_text">' + _("ชนิดรูปที่อนุญาต: jpg , png, tiff, gif, bmp ขนาดไม่เกิน 100 MB ต่อหนึ่งไฟล์ ไม่จำกัดจำนวนไฟล์") + '</div>' +
				...
				'<tr>' +
					'<td class="plupload_cell plupload_file_name">' + _('ชื่อไฟล์') + '</td>' +
					'<td class="plupload_cell plupload_file_status">' + _('สถานะ') + '</td>' +
					'<td class="plupload_cell plupload_file_size">' + _('ขนาด') + '</td>' +
					'<td class="plupload_cell plupload_file_action">&nbsp;</td>' +
				'</tr>' +
				'</table>' +

				'<div class="plupload_content">' +
					'<div class="plupload_droptext">' + _("ลากไฟล์มาที่นี่") + '</div>' +
				...
				'<tr>' +
					'<td class="plupload_cell plupload_file_name">' +
						'<div class="plupload_buttons"><!-- Visible -->' +
							'<a class="plupload_button plupload_add">' + _("เพิ่มไฟล์") + '</a>&nbsp;' +
							'<a class="plupload_button plupload_start">' + _("เริ่มอัพโหลด") + '</a>&nbsp;' +
							'<a class="plupload_button plupload_stop plupload_hidden">'+_("หยุดอัพโหลด") + '</a>&nbsp;' +
						'</div>' +
	...
	*/
	...
	_handleState: function() {
		...
		if (plupload.STARTED === up.state) {
			...
			$('.plupload_upload_status', this.element).html(plupload.sprintf(_('อัพโหลดแล้ว %d/%d ไฟล์'), up.total.uploaded, up.files.length));
		...
		if (up.total.queued === 0) {
			$('.ui-button-text', this.browse_button).html(_('เพิ่มไฟล์'));
		} else {
			$('.ui-button-text', this.browse_button).html(plupload.sprintf(_('%d ไฟล์ที่อยู่ในคิว'), up.total.queued));
		}
	...
	_updateTotalProgress: function() {
		...
		this.element
			...
			.find('.plupload_upload_status')
				.html(plupload.sprintf(_('อัพโหลดแล้ว %d/%d ไฟล์'), up.total.uploaded, up.files.length));
	},
